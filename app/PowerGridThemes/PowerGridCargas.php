...
class PowerGridCargas extends Tailwind
{
    public string $name = 'tailwind';

    public function table(): Table
    {
        return Theme::table('rounded-lg min-w-full border border-slate-400 dark:bg-slate-600 dark:border-slate-500')
            ->div('my-3 overflow-x-auto bg-white shadow-sm rounded-lg overflow-y-auto relative')
            ->thead('shadow-sm bg-slate-300 dark:bg-slate-800 border border-slate-200 dark:border-slate-500')
            ...
            ->trBody('border border-slate-400 dark:border-slate-400 hover:bg-slate-50 dark:bg-slate-700 dark:odd:bg-slate-800 dark:odd:hover:bg-slate-900 dark:hover:bg-slate-700')
            ->tdBody('px-3 py-2 whitespace-nowrap dark:text-slate-200')
            ->tdBodyTotalColumns('px-3 py-2 whitespace-nowrap dark:text-slate-200 text-xs text-slate-600 text-right space-y-2');
    }
    ...
    public function footer(): Footer
    {
        return Theme::footer()
            ...
            ->select('block appearance-none bg-korange border border-slate-300 text-slate-700 py-2 px-3 pr-8 rounded leading-tight focus:outline-none focus:bg-white focus:border-slate-500  dark:bg-slate-600 dark:text-slate-200 dark:placeholder-slate-200 dark:border-slate-500');
    }
    ...
    public function actions(): Actions
    {
        return Theme::actions()
            ...
            ->rowsBtn('focus:outline-none text-xs py-2.5 px-5 rounded border');
    }
